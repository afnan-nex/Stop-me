 ...
 *  - Blocking: full-screen block when time is exhausted
 *  - Challenge: full-screen unlock challenge flow
 *
 * All overlays are backed by [ComposeView] with a managed [ServiceLifecycleOwner].
 */
class OverlayManager(private val context: Context) {
            ...
            blockingView = createComposeView {
                ...
                StopMeTheme {
                    BlockingOverlayContent(
                        ...
                        unlockEnabled = state.unlockEnabled,
                        onUnlockTap = state.onUnlockTap
                    )
                }
            }
            windowManager.addView(blockingView, blockingParams())
        } else {
            // Update state for existing view (e.g. re-opened blocked app)
    ...
    private fun blockingParams() = baseParams(
        ...
        // Focusable to intercept back button
    )

    private fun challengeParams() = baseParams(
        flags = WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
        // Focusable to support keyboard input
    )
}
